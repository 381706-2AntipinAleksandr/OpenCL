...
__kernel void MatrixMultiplicationBlock(__global float* a, __global float* b, __global float* c, const unsigned int m, const unsigned int n) {
    ...
    for (int p = 0; p < m / loc_size_x; ++p) {
        ...
        A[loc_id_y * loc_size_x + loc_id_x] = a[tid_y * m + p * loc_size_x + loc_id_x];
        B[loc_id_y * loc_size_x + loc_id_x] = b[(p * loc_size_y + loc_id_y) * n + tid_x];
