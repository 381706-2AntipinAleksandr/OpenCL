...
int main(int argc, char** argv) {
    ...
    std::cout << "Float compare - " << (IsEqual(c_seq, c_par_gpu, n * n) == true ? "true" : "false") << std::endl;

    ParallelOpenCLMatrixMultiplicationOptimized(argv[0], 16, a_par_gpu_opt, b_par_gpu_opt, c_par_gpu_opt, m, n);
    std::cout << "Float compare optimized - " << (IsEqual(c_seq, c_par_gpu_opt, n * n) == true ? "true" : "false") << std::endl;

    ParallelOpenCLMatrixMultiplicationImage(argv[0], 16, a_par_gpu_img, b_par_gpu_img, c_par_gpu_img, m, n);
    std::cout << "Float compare image - " << (IsEqual(c_seq, c_par_gpu_img, n * n) == true ? "true" : "false") << std::endl;
